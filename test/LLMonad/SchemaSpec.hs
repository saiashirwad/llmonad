...
spec = do
  describe "Schema Engine (Tier 1: Feature Coverage)" $ do
    ...
    it "describes lists with items schema" $ do
      toSchema @[Int] `shouldBe`
        object ["type" .= ("array" :: Text), "items" .= object ["type" .= ("integer" :: Text)]]
  ...
  describe "Schema Engine (Tier 2: Boundary & Corner Cases)" $ do
    ...
    it "derives positional tuple schemas" $ do
      toSchema @(Int, Text, Bool) `shouldBe`
        object
          [ "type" .= ("array" :: Text)
    ...
    it "derives 4-element tuple schema" $ do
      toSchema @(Int, Double, Text, Scientific) `shouldBe`
        object
          [ "type" .= ("array" :: Text)
    ...
    it "derives minItems: 1 for NonEmpty lists" $ do
      toSchema @(NonEmpty Text) `shouldBe`
        object ["type" .= ("array" :: Text), "items" .= object ["type" .= ("string" :: Text)], "minItems" .= (1 :: Int)]

    it "derives uniqueItems: true for Set containers" $ do
      toSchema @(Set.Set Text) `shouldBe`
        object ["type" .= ("array" :: Text), "items" .= object ["type" .= ("string" :: Text)], "uniqueItems" .= True]

    it "derives dictionary object schema for Map Text v" $ do
      toSchema @(Map.Map Text Int) `shouldBe`
        object ["type" .= ("object" :: Text), "additionalProperties" .= object ["type" .= ("integer" :: Text)]]
