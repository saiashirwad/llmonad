...
withTrace emitTrace = interpose_ $ \case
  ChatRound p fmt specs choice -> do
    sys <- getSystem
    hist <- getHistory
    liftIO $ emitTrace TraceRequest
      { trModel = Model "active"
      , trSystem = sys
      , trMessages = hist
      }
    resp <- send (ChatRound p fmt specs choice) `E.catch` \(err :: LLMError) -> do
      liftIO $ emitTrace (TraceError err)
      E.throwIO err
    liftIO $ emitTrace TraceResponse
  ...
  StreamRound p fmt specs cb -> do
    ...
    resp <- send (StreamRound p fmt specs cb) `E.catch` \(err :: LLMError) -> do
      liftIO $ emitTrace (TraceError err)
      E.throwIO err
    liftIO $ emitTrace TraceResponse
      { trText = crspText resp
  ...
  PushMessage msg -> do
    case msg of
      ToolMsg cid content -> do
        ...
        liftIO $ emitTrace TraceToolExecuted
          { trToolName = tName
          , trToolOk = isOk
