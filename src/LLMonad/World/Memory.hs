  ...
  SearchFiles opts -> do
    ...
    if not (isMemDir (mwsFiles st) searchPrefix)
      then throw (WorldDirectoryNotFound (soSearchDir opts))
        ...
        let matches = concatMap (\(path, content) ->
...
getChildEntries dirPrefix allFiles =
  let relevantFiles = if null dirPrefix
                        then allFiles
...
getAllMemEntries prefix allFiles =
  let filtered = if null prefix then allFiles else [ f | f <- allFiles, (prefix ++ "/") `isPrefixOf` f ]
      ...
      dirEntries = [ (d, True, pathDepth (if null prefix then d else makeRelative prefix d))
                   | d <- allDirs
...
simulateCommand spec st = case cmdProgram spec of
  ...
  "cat" ->
    let contents = [ Map.findWithDefault "" (canonicalMemKey (mwsWorkingDir st) (T.unpack arg)) (mwsFiles st)
                   | arg <- cmdArgs spec
                   ]
...
simulateShell inner _ st
  | "echo " `T.isPrefixOf` inner =
      let msg = T.drop 5 inner
          cleanMsg = if (T.isPrefixOf "\"" msg && T.isSuffixOf "\"" msg) || (T.isPrefixOf "'" msg && T.isSuffixOf "'" msg)
